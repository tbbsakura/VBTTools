...
3. 次にドライバ署名確認をクリアするため、コマンドプロンプトを管理者権限で起動します。
C以外のドライブに展開した場合はそのドライブに移動してます。<br>
画像はZドライブなので Z: [Enter]で移動しています。<br>
さらに展開したディレクトリにcdコマンドで移動します。<br>（移動先ディレクトリ名は Exlporerの上部でコピーし、コマンドプロンプトを右クリックすれば貼り付けできます）<br>
移動できたら installCert.bat[enter]と押す (全部打つのがだるい場合 ins[tab][enter]でいけると思います。）
